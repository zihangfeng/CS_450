...
int main(int argc, char *argv[])
{
   ...
   struct in_addr **pptr = (struct in_addr **)hostIP->h_addr_list;
	...
	else
	{
		...
		int currentPack = 0, numOfPacketsAck = 0;// numOfPacketsSent tells how many packets have been saved correctly by the server
			...
			// timer should start at here
                        ...
                        {
                        	if(headerAck.ackNumber)// it tells how many packets have been saved correctly by the server
                        	{
					// if everything goes correctly
					// curretPack should stay at 1
					if(!(headerAck.ackNumber < numOfPacketsAck))
					{
						currentPack = headerAck.ackNumber - numOfPacketsAck;
						if(currentPack > 0)// some packets have been received accurately
						{
							currentPack = winSize - currentPack;
						}
						numOfPacketsAck = headerAck.ackNumber;
					}
				}
                        }
		}// while loop
