...
{
int rn, filefd, count, number_garble = 0;
uint16_t checksumVal;
CS450Header headerAck;
int8_t toSaveFile = pack.header.saveFile;
uint32_t numBlockRec, totalBlock = pack.header.nTotalBytes/pack.header.nbytes;


if(pack.header.nTotalBytes%pack.header.nTotalBytes)
{
	totalBlock += 1;
}
	...
	while(numBlockRec <= totalBlock)
        {
			...
			headerAck.sequenceNumber = numBlockRec;
			...
			// check if the pack is garbled
                        if(calcCheckSum(&pack, PacketSize) == checksumVal)
                       	{
				// to check if it is the pack falls into the specific range
				if(numBlockRec <= pack.header.sequenceNumber && (numBlockRec + WINSIZE) > pack.header.sequenceNumber)
				{
					if(numBlockRec == pack.header.sequenceNumber)
					{
						if(toSaveFile)
						{
						...
						headerAck.sequenceNumber = pack.header.sequenceNumber;
						ackSend(UDPsockfd, headerAck, clntAddr, clntLen);
						buf[numBlockRec%WINSIZE].header.ackNumber = 0;
						numBlockRec++;
					}
					else
					{
						printf("It has unexpected pack.\n");
						// to check if the out of order pack is acknowledged
						if(!buf[pack.header.sequenceNumber%WINSIZE].header.ackNumber)
						{
							pack.header.ackNumber = 1;
							memcpy(buf + pack.header.sequenceNumber%WINSIZE, &pack, PacketSize);
							headerAck.ackNumber = 1;
							headerAck.sequenceNumber = pack.header.sequenceNumber;
                                			ackSend(UDPsockfd, headerAck, clntAddr, clntLen);
							...
							while(count < WINSIZE)
							{
								// check if it is the expected pack
								if(!buf[numBlockRec%WINSIZE].header.ackNumber)
								{
                                        				headerAck.ackNumber = 0;
                                        				headerAck.sequenceNumber = numBlockRec;
                                        				ackSend(UDPsockfd, headerAck, clntAddr, clntLen);
									break;
                                        			...
                                        			{
                                                			write(filefd, buf[numBlockRec%WINSIZE].data, buf[numBlockRec%WINSIZE].header.nbytes);
                                        			}
			...
			{
                               	number_garble++;
                               	headerAck.ackNumber = 0;// get the wrong package from the client
				headerAck.sequenceNumber = numBlockRec;
                                ackSend(UDPsockfd, headerAck, clntAddr, clntLen);
                                printf("Garbled.");
			}
